...
{
    double bias;
    double VAR_BIAS = 0.03; //txy 20210314
...
static void ud_state(const obsd_t *obs, int n, const nav_t *nav, double *x,
                    double *P, int nx, ssat_t *ssat,double *pos)
{
    gtime_t time;
    double tt;
    int i,sat;
    double MAXGAP_BIAS = 30;     //txy
    for (i=0;i<n;i++) {
        sat = obs[i].sat;
        time=ssat[sat-1].time;
        
        if (!time.time) {
            init_iono(obs+i,nav,x,P,nx,pos);
...
static int res_iono(const obsd_t *obs, int n, const double *azel,
                    const double *x, int nx, double *v, double *H, double *R,double *pos)
{
...
static int est_iono(obs_t *obs, nav_t *nav, double *rr, FILE *fp)
{
    ...
    for (i=0;i<obs->n;i++) {
        ...
        ud_state(obs+i,n,azel,x,P,nx,ssat,pos);
        
        /* ionosphere residuals */
        if ((nv=res_iono(obs+i,n,azel,x,nx,v,H,R,pos))<=0) break;
        
        /* filter */
        if ((info=filter(x,P,H,v,R,nx,nv))) break;
        
        /* output ionopshere parameters */
        out_iono(obs[i].time,x,P,nx,fp,ssat);
    }
